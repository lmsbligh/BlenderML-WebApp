...
def delete_model():
    ...
    try:
        ...
        if not model_full_path.startswith(MODELS_DIR_PATH):
            print(f"error, path {model_full_path} is not within models folder.")
            abort(403)
        else:
...
def delete_dataset(model_ID, checkpoint_ID):
    MODELS_DIR_PATH = current_app.config["MODELS_DIR_PATH"]
    checkpoint_full_path = os.path.realpath(os.path.join(MODELS_DIR_PATH, model_ID, checkpoint_ID))
    
    try:
        if not checkpoint_full_path.startswith(MODELS_DIR_PATH):
            print(f"error, path {checkpoint_full_path} is not within datasets folder!!")
            abort(403)
        else:
