...
def save_dataset_props(split_dir, generate_profile, split):
    ...
    match split:
        case "train":
            size = math.ceil(generate_profile['datasetSize']*((100 - int(generate_profile['TestSetPercentage']) - int(generate_profile['CVPercentage']))/100))
        case "CV":
            size = math.ceil(generate_profile['datasetSize']*(int(generate_profile['CVPercentage'])/100))
        case "test":
            size = math.ceil(generate_profile['datasetSize']*(int(generate_profile['TestSetPercentage'])/100))
